...
export const BlurTimelineOverlay: React.FC<BlurTimelineOverlayProps> = ({
  ...
  let maxPossibleOverlays = Math.floor(availableHeight / (overlayHeight + overlaySpacing));
  ...
  if (allRegions.length > maxPossibleOverlays && allRegions.length > 0) {
    ...
    overlayHeight = Math.max(6, Math.floor((availableHeight - (allRegions.length - 1) * overlaySpacing) / allRegions.length));
    maxPossibleOverlays = Math.floor(availableHeight / (overlayHeight + overlaySpacing));
  }
  ...
  useEffect(() => {
    if (activeRegionId && activeVideoId && allRegions.length > 0) {
      const activeIndex = allRegions.findIndex(
        item => item.region.id === activeRegionId && item.videoId === activeVideoId
      );
      if (activeIndex !== -1) {
        // Check if active region is visible
        if (activeIndex < startIndex || activeIndex >= startIndex + maxOverlays) {
          // Scroll to show the active region
          const newStartIndex = Math.max(0, Math.min(activeIndex, allRegions.length - maxOverlays));
  ...
  return (
    ...
    >
      ...
      {allRegions.length > maxOverlays && (
        <div className="absolute -right-6 top-0 flex flex-col space-y-0.5" data-blur-controls="true">
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              setStartIndex(Math.min(allRegions.length - maxOverlays, startIndex + 1));
            }}
      ...
      {allRegions.length > maxOverlays && (
        ...
        >
          {startIndex + 1}-{Math.min(startIndex + maxOverlays, allRegions.length)}/{allRegions.length}
        </div>
      )}

      {visibleRegions.map((item, index) => {
        const { region, videoId, videoLeft, videoWidth } = item;
        const isActive = region.id === activeRegionId && videoId === activeVideoId;
          ...
          >
            <div className="flex items-center justify-between h-full px-1">
              <div className="flex items-center space-x-0.5">
                {region.shape === "circle" ? (
                  <Circle className={`w-1.5 h-1.5 flex-shrink-0 drop-shadow-sm ${isActive ? 'text-white' : 'text-white'}`} />
                ) : (
                  <Square className={`w-1.5 h-1.5 flex-shrink-0 drop-shadow-sm ${isActive ? 'text-white' : 'text-white'}`} />
                )}
                <span className={`text-xs font-medium truncate drop-shadow-sm ${isActive ? 'text-white' : 'text-white'}`}>
                  #{startIndex + index + 1}
                </span>
                {isActive && (
              ...
              <span className={`text-xs font-medium rounded text-center min-w-[16px] px-0.5 py-0.5 ${
                isActive 
