...
const extractVideoFramesProgressively = (
  ...
  Promise.all(
    ...
    .then(() => {
      ...
      const optimalFrames = Math.max(
        8,
        Math.min(120, Math.round(segmentWidthPx / 10))
      );
      ...
      const extractFrame = async (
        ...
        return new Promise((resolve) => {
          ...
          ); // Increased timeout: 4s, 6s, 8s for concurrent processing
            ...
            const batchPromises = batch.map(async (time, index) => {
              ...
              if (index > 0) {
                await new Promise((resolve) =>
                  setTimeout(resolve, Math.min(index * 10, 100))
                );
              }
      ...
      // Execute both passes with proper error handling
      (async () => {
        try {
          // Pass 0: low-res
          await processPass(0);
          ...
          URL.revokeObjectURL(videoUrl);
          onComplete(); // Still call completion to unblock the process
        }
      })();
    })
    .catch((error) => {
      console.error("Video metadata loading failed:", error);
      // Clean up resources and call completion to unblock
      URL.revokeObjectURL(videoUrl);
      onComplete();
    });
};
...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  const zoomOut = useCallback(() => {
    ...
    const minZoomForFullVideo =
      totalDuration > 0 ? BASELINE_DURATION / totalDuration : 0.25;
    ...
    (e: React.MouseEvent) => {
      ...
      const newStart = Math.max(
        ...
        Math.min(
          ...
          clickPercentage * totalDuration - visibleDuration / 2
        )
      );
  ...
  useEffect(() => {
    if (totalDuration > 0) {
      ...
      const currentCenter =
        viewportEnd === 0 ? 0 : (viewportStart + viewportEnd) / 2;
  ...
  useEffect(() => {
    ...
    const extractFramesForVideos = async () => {
      ...
      const processVideoWithQueue = async (video: any) => {
        ...
        if (
          !frameExtractionRef.current.get(video.id) &&
              ...
              extractVideoFramesProgressively(
                ...
                (frame) => {
                  ...
                  setVideoFrames((prev) => {
                    ...
                    let insertIdx = currentFrames.findIndex(
                      (f) => f && Math.abs(f.time - frame.time) < 1e-3
                    );
                    let newFrames;
                    if (insertIdx !== -1) {
      ...
      try {
        ...
        const failedExtractions = results.filter(
          (r) => r.status === "rejected"
        );
  ...
  const calculateTimeToPixels = useCallback(
    (time: number) => {
  ...
  const calculatePixelsToTime = useCallback(
    (pixels: number) => {
    ...
    (video: VideoSequenceItem) => {
      ...
      if (totalDuration > 0 && totalDuration < 25) {
        ...
        widthPixels = Math.max(
          30 * zoomLevel,
          calculateTimeToPixels(effectiveDuration)
        ); // Minimum width scales with zoom
      }
  ...
  const handleTimelineClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const clickTimeFromPixels =
        calculatePixelsToTime(clickPixelOffset) + viewportStart;

      // Clamp to valid time range
      const clickTime = Math.max(
        0,
        Math.min(totalDuration, clickTimeFromPixels)
      );
  ...
  const handleTimelineMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (
        ...
        target.closest(".video-segment") ||
        target.closest(".add-sequence-btn")
      ) {
        return;
      }
      ...
      if (isDragging && clickPosition) {
        ...
        if (hasDraggedEnough) {
          ...
          const newTime = Math.max(
            ...
            Math.min(
              totalDuration,
              calculatePixelsToTime(movePixelOffset) + viewportStart
            )
          );
      ...
      const handleGlobalMouseMove = (e: MouseEvent) => {
        ...
        if (scrollbarElement) {
          ...
          const movePercentage = Math.max(
            0,
            Math.min(1, moveX / scrollbarRect.width)
          );
          ...
          const newStart = Math.max(
            ...
            Math.min(
              totalDuration - visibleDuration,
              movePercentage * (totalDuration - visibleDuration)
            )
          );
          const newEnd = newStart + visibleDuration;
  ...
  const handleVideoClick = useCallback(
    (video: VideoSequenceItem, e: React.MouseEvent) => {
      ...
      const segmentClickPercentage = Math.max(
        0,
        Math.min(1, clickX / segmentRect.width)
      );
      ...
      if (totalDuration > 0 && totalDuration < 25) {
        ...
        absoluteTime =
          videoStartTime + segmentClickPercentage * effectiveDuration;
      }
  ...
  const renderVideoSegment = useCallback(
    (
      ...
      const segmentWidthPx = Math.max(
        MIN_FRAME_CONTAINER_WIDTH,
        effectiveDuration * PIXELS_PER_SECOND
      );
        ...
        frameNaturalWidth =
          loadedFrameCount > 0
            ? Math.max(
                MIN_FRAME_CONTAINER_WIDTH,
                Math.min(
                  MAX_FRAME_CONTAINER_WIDTH,
                  segmentWidthPx / loadedFrameCount
                )
              )
            : MIN_FRAME_CONTAINER_WIDTH;
      }
      ...
      return (
        ...
        >
          ...
          <div
            className={`absolute inset-0 bg-gray-200 ${loadedFrameCount === 0 ? "animate-pulse" : ""}`}
          ...
          <div
            ...
            style={{
              ...
              width:
                totalDuration > 0 && totalDuration < 25
                ...
                <div
                  ...
                  onClick={(e) => {
                    ...
                    const percentage = Math.max(
                      0,
                      Math.min(1, clickX / containerRect.width)
                    );
                    const visibleDuration = viewportEnd - viewportStart;
                    const visiblePixels =
                      calculateTimeToPixels(visibleDuration);
                    const clickPixelOffset = percentage * visiblePixels;
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-4">
          ...
          <button
            ...
            disabled={(() => {
              ...
              if (totalDuration > 0 && totalDuration < BASELINE_DURATION) {
                ...
                absoluteMinZoom = Math.min(
                  0.25,
                  Math.max(0.05, minZoomForFullVideo)
                );
              }
      ...
      >
          ...
          {hoverPosition !== null && totalDuration > 0 && !isDragging && (
            ...
            >
              <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-gray-700 text-gray-300 text-xs px-2 py-0.5 rounded whitespace-nowrap font-mono">
                {formatTime(
                  viewportStart +
                    (hoverPosition / 100) * (viewportEnd - viewportStart)
                )}
              </div>
            </div>
          )}
        ...
        {/* Timeline Ruler */}
        <div
          className={`absolute left-0 right-0 h-6 bg-gray-850 border-t border-gray-700 z-50 ${
        ...
        >
          {(() => {
            ...
            return Array.from({ length: endIndex - startIndex + 1 }, (_, i) => {
              ...
              const visiblePixels = calculateTimeToPixels(visibleDuration);
              const position =
                ((timePixels - viewportStartPixels) / visiblePixels) * 100;
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`absolute top-3 text-[11px] text-gray-300 font-mono bg-gray-850 px-1 border border-gray-700 rounded z-50 ${
                      isFirst
                        ? "left-0"
                        : isLast
