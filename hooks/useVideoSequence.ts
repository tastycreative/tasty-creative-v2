...
export const useVideoSequence = () => {
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            video.onloadedmetadata = () => {
              ...
              if (gridId) {
                ...
                const existingGridVideos = videos.filter(
                  (v) => v.gridId === gridId
                );
                const newGridVideos = newVideos.filter(
                  (v) => v.gridId === gridId
                );
                ...
                const allSingleVideos = [
                  ...existingSingleVideos,
                  ...newSingleVideos,
                ];
                ...
                if (allSingleVideos.length > 0) {
                  ...
                  startTime = Math.max(
                    ...allSingleVideos.map((v) => v.endTime)
                  );
                } else {
        ...
        setVideos((prev) => {
          ...
          if (gridId) {
            ...
            for (const v of singleVideos) {
              v.startTime = runningTime;
              v.endTime = runningTime + v.duration;
              console.log("Updated video timing:", {
                name: v.file.name,
    ...
    (id: string) => {
      setVideos((prev) => {
        ...
        if (videoToRemove?.gridId) {
          const gridVideos = updated.filter(
            (v) => v.gridId === videoToRemove.gridId
          );
          let runningTime = 0;
          for (const v of gridVideos) {
            v.startTime = runningTime;
          ...
          return updated.map((v) => {
            if (v.gridId === videoToRemove.gridId) {
              ...
              return updated_grid
                ? {
                    ...v,
                    startTime: updated_grid.startTime,
                    endTime: updated_grid.endTime,
                  }
                : v;
            }
            return v;
  ...
  const play = useCallback(() => {
    if (!isPlaying) {
      ...
      }, 33); // ~30fps timeline updates to match preview and reduce desync/white frames
    }
