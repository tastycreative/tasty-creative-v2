...
export default function AccountDetailsPage() {
  ...
  const [profileVisitors, setProfileVisitors] =
    useState<ProfileVisitors | null>(null);
  ...
  const fetchAccountDetails = async () => {
    try {
      ...
      try {
        ...
        if (accountsResponse.ok) {
          ...
          if (currentAccount) {
            ...
            const realAccountId =
              currentAccount.id ||
              currentAccount.onlyfans_user_data?.id ||
              accountId;
      ...
      if (accountData) {
        realAccountId =
          accountData.id || accountData.onlyfans_user_data?.id || accountId;
      }
      ...
      const endpointConfigs = [
        {
          name: "chats",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=chats`,
        },
        {
          name: "active-fans",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=active-fans`,
        },
        {
          name: "expired-fans",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=expired-fans`,
        },
        {
          name: "vault-media",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=vault-media`,
        },
        {
          name: "earnings",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=earnings&startDate=${startDate}&endDate=${endDate}`,
        },
        {
          name: "profile-visitors",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=profile-visitors&startDate=${startDate}&endDate=${endDate}`,
        },
        {
          name: "transactions",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=transactions&startDate=${startDate}&endDate=${endDate}`,
        },
        {
          name: "tracking-links",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=tracking-links`,
        },
        {
          name: "profile-details",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=profile-details`,
        },
        {
          name: "account-balances",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=account-balances`,
        },
        {
          name: "vault-lists",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=vault-lists`,
        },
        {
          name: "mass-messaging",
          url: `/api/onlyfans/models?accountId=${encodeURIComponent(realAccountId)}&endpoint=mass-messaging`,
        },
      ];
        ...
        if (response.status === "fulfilled") {
          ...
          switch (endpoint) {
            case "chats":
              ...
              break;
            case "active-fans":
              const activeFansArray = Array.isArray(data)
                ? data
              ...
              break;
            case "expired-fans":
              const expiredFansArray = Array.isArray(data)
                ? data
                : data.data?.list || data.list || data.data || [];
              ...
              break;
            case "vault-media":
              const vaultMediaArray = Array.isArray(data)
                ? data
              ...
              break;
            case "vault-lists":
              const vaultListsArray = Array.isArray(data)
                ? data
            ...
            case "earnings":
              ...
              const transformedEarnings = {
                ...
                pending_balance: 0,
                monthly_earnings:
                  earningsData.total?.chartAmount?.[0]?.count || 0,
                daily_earnings: {},
            ...
            case "profile-visitors":
              ...
              const transformedVisitors = {
                total_visitors:
                  visitorsData.chart?.visitors?.reduce(
                    (sum: number, visitor: any) => sum + visitor.count,
                    0
                  ) || 0,
            ...
            case "transactions":
              ...
              break;
            case "tracking-links":
              const trackingLinksArray = Array.isArray(data)
                ? data
            ...
            case "profile-details":
              ...
              if (
                profileDetailsData &&
                Object.keys(profileDetailsData).length > 0
              ) {
            ...
            case "account-balances":
              if (data && typeof data === "object") {
                setEarnings((prev) => ({
                  ...prev,
                  current_balance:
                    data.current_balance || prev?.current_balance,
                  pending_balance:
                    data.pending_balance || prev?.pending_balance,
              ...
              break;
            case "mass-messaging":
              const massMessagesArray = Array.isArray(data)
                ? data
                : data.data?.list || data.list || data.data || [];
      ...
      try {
        const leaderboardResponse = await fetch(
          `/api/onlyfans/models?endpoint=mass-messaging-leaderboard`
        );
        if (leaderboardResponse.ok) {
          ...
          console.error(
            "Failed to fetch mass messaging leaderboard:",
        ...
        <div className="flex items-center gap-4">
          {(accountData?.avatar || accountData?.onlyfans_user_data?.avatar) && (
            <div className="relative group">
              {/* Background gradient - placed first, behind the image */}
              ...
              {/* Image - placed second, in front of background */}
              <img
                src={`/api/image-proxy?url=${encodeURIComponent(accountData?.avatar || accountData?.onlyfans_user_data?.avatar)}`}
                alt={
                  accountData?.display_name || accountData?.name || "Account"
                }
                className="relative w-16 h-16 rounded-full object-cover border-2 border-pink-200"
              />
            </div>
          )}
          <div>
            ...
            <div className="mt-2 text-sm text-gray-600 flex gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant={
                    accountData?.is_authenticated ? "default" : "secondary"
                  }
                  className={
                    accountData?.is_authenticated ? "bg-black text-white" : ""
                  }
                >
                  {accountData?.is_authenticated
                    ? "Authenticated"
                    : "Not Authenticated"}
                </Badge>
              </div>
