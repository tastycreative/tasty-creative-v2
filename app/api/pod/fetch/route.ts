  ...
  try {
    ...
    try {
      ...
      try {
          ...
          if (sheet.data && sheet.data.length > 0) {
            ...
            if (gridData.rowData && gridData.rowData.length > 0) {
              ...
              if (row.values && row.values.length > 0) {
                ...
                else if (cell && cell.chipRuns && Array.isArray(cell.chipRuns)) {
                  for (const chipRun of cell.chipRuns) {
                    if (chipRun.chip && chipRun.chip.richLinkProperties && chipRun.chip.richLinkProperties.uri) {
        ...
        console.error('Error extracting hyperlink from cell data:', hyperlinkError);
      }
        ...
        try {
          ...
          if (linkedSpreadsheetId) {
            ...
            for (let i = 0; i < rowsToSearchForHeader; i++) {
              if (columnKValues[i] && columnKValues[i][0]) {
                ...
                if (cellValue.toLowerCase().includes('sheet links')) {
                  ...
                  console.log('Found Sheet Links at row:', i + 1, 'with value:', cellValue);
                  break;
                }
              }
            }
            ...
            if (sheetLinksRowIndex !== -1) {
              console.log('Starting to collect all links from row:', sheetLinksRowIndex + 2);
              ...
              const sheetLinksGridResponse = await sheets.spreadsheets.get({
                spreadsheetId: linkedSpreadsheetId,
                includeGridData: true,
                ranges: [`K${sheetLinksRowIndex + 2}:K${Math.min(columnKValues.length, sheetLinksRowIndex + 50)}`], // Get up to 50 rows of data
              });
              
              // Extract hyperlinks from grid data
              if (sheetLinksGridResponse.data.sheets && sheetLinksGridResponse.data.sheets.length > 0) {
                  ...
                  if (gridData.rowData) {
                    gridData.rowData.forEach((row, index) => {
                      if (row.values && row.values.length > 0) {
                        ...
                        let displayText = '';
                        if (columnKValues[actualRowIndex] && columnKValues[actualRowIndex][0]) {
                          displayText = columnKValues[actualRowIndex][0].toString().trim();
                        }
                        
                        // Split display text by | to get individual names
                        const displayNames = displayText.split('|').map(name => name.trim()).filter(name => name);
                        ...
                        if (cell && cell.chipRuns && Array.isArray(cell.chipRuns)) {
                          for (const chipRun of cell.chipRuns) {
                            if (chipRun.chip && chipRun.chip.richLinkProperties && chipRun.chip.richLinkProperties.uri) {
                              smartChipUrls.push(chipRun.chip.richLinkProperties.uri);
                            }
                          }
                        }
                        
                        // Create link entries - pair display names with URLs
                        if (displayNames.length > 0 && smartChipUrls.length > 0) {
                          ...
                          if (displayNames.length > 1 && smartChipUrls.length > 1) {
                            displayNames.forEach((name, nameIndex) => {
                              const linkEntry = {
                                name: name,
                                url: smartChipUrls[nameIndex] || smartChipUrls[0] || '', // Use corresponding URL or first URL as fallback
                        ...
                        if (cellLinks.length === 0 && smartChipUrls.length > 0) {
                        ...
                        cellLinks.forEach(link => {
                          ...
                          console.log(`Row ${actualRowIndex + 1}: Added link: "${link.name}" -> ${link.url || 'no URL'}`);
                        });
                      }
                    });
                  }
              ...
              console.log('Sheet Links header not found in column K within first', rowsToSearchForHeader, 'rows');
            }
          }
