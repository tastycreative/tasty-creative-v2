  ...
  try {
    ...
    if (!fileId && !thumbnailLink) {
      return NextResponse.json(
        { error: "Missing fileId or thumbnailLink" },
        { status: 400 }
      );
    }
    ...
    if (thumbnailLink && !fileId) {
      ...
      return NextResponse.json(
        { error: "thumbnailLink not supported, use fileId instead" },
        { status: 400 }
      );
    }
    ...
    if (fileId) {
      ...
      if (mimeType.startsWith("video/") && thumbnailLinkFromMetadata) {
        ...
        if (size === "large") {
          ...
          enhancedThumbnailUrl = thumbnailLinkFromMetadata
            .replace(/=s\d+/, "=s1920")
            .replace(/=w\d+-h\d+/, "=w1920");
        } else if (size !== "default" && !isNaN(parseInt(size))) {
          // Custom size specified
          enhancedThumbnailUrl = thumbnailLinkFromMetadata
            .replace(/=s\d+/, `=s${size}`)
            .replace(/=w\d+-h\d+/, `=w${size}`);
        }
    ...
    return NextResponse.json(
      { error: "No valid file ID provided" },
      { status: 400 }
    );
  } catch (error: any) {
