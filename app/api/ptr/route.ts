...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      try {
        ...
        await sheets.spreadsheets.values.update({
          ...
          requestBody: {
            values: [['Item ID', 'Title', 'Creator', 'Is PTR', 'Last Sent', 'Send Count', 'Revenue', 'Buys', 'Updated At', 'Updated By', 'Created At', 'Notes']]
          }
        });
      } catch (createError) {
        console.error('Error creating PTR Tracking sheet:', createError);
        return NextResponse.json({ error: 'Failed to create PTR tracking sheet' }, { status: 500 });
      }
    }
    ...
    const existingRowIndex = rows.findIndex((row, index) => index > 0 && row[0] === itemId);
    ...
    return NextResponse.json({ 
      success: true, 
      message: isPTR ? 'Marked as PTR successfully' : 'Removed PTR successfully',
      isPTR
    });

  } catch (error) {
    console.error('Error toggling PTR:', error);
    return NextResponse.json({ error: 'Failed to toggle PTR' }, { status: 500 });
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch PTR items' }, { status: 500 });
  }
}