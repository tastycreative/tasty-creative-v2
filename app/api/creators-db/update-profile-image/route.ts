...
function generateProfileImageS3Key(
  modelName: string,
  originalName: string
): string {
...
async function deleteExistingProfileImages(modelName: string): Promise<void> {
  try {
    ...
    if (listResponse.Contents && listResponse.Contents.length > 0) {
      console.log(
        `🗑️ Found ${listResponse.Contents.length} existing image(s) to delete`
      );
    ...
    console.error(
      "⚠️ Error deleting existing profile images (continuing anyway):",
      error
    );
...
async function updateGoogleSheet(
  ...
  try {
    const GOOGLE_DRIVE_SHEET_MODEL_NAMES =
      process.env.GOOGLE_DRIVE_SHEET_MODEL_NAMES;

    if (!GOOGLE_DRIVE_SHEET_MODEL_NAMES) {
      console.log(
        "⚠️ GOOGLE_DRIVE_SHEET_MODEL_NAMES not configured, skipping Google Sheets update"
      );
      return;
    }
    ...
    if (!rowId) {
      console.log(
        "⚠️ No row_id found in ClientModel, cannot update Google Sheet"
      );
      return;
    }
    ...
    const webhookUrl =
      "http://n8n.tastycreative.xyz/webhook/1f9c704a-f940-4a02-95aa-20164df19c25";
    const webhookData = {
    ...
    console.log(
      `✅ Updated Google Sheet via webhook - Range: Profile Link${rowId}, Value: ${imageUrl}`,
      result
    );
  } catch (error) {
  ...
  try {
    ...
    if (session.user.role !== "ADMIN" && session.user.role !== "MODERATOR") {
      return NextResponse.json(
        { error: "Not authorized - admin or moderator access required" },
        { status: 403 }
      );
    }
