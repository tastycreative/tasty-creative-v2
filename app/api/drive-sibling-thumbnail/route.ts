...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const sibling = listRes.data.files?.find(f => f.name === fileName && f.id !== fileId);
    if (sibling) {
      ...
      return new Response(JSON.stringify({ error: "No sibling file with matching name found" }), { status: 404 });
    }
  } catch (error: any) {
