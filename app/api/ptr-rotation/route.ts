  ...
  try {
    ...
    try {
      ...
      performanceHistory = parsed.map((entry: any) => ({
        ...
        result: ["good", "bad", "pending"].includes(entry.result)
          ? entry.result
          : "pending",
      }));
    } catch (e) {
    ...
    const validResult: "good" | "bad" | "pending" = [
      "good",
      "bad",
      "pending",
    ].includes(result)
    ...
    console.log(
      `✅ PTR marked as sent: ${itemId} (usage count: ${currentUsageCount + 1})`
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const isFirstRowData =
      firstRow &&
      firstRow[0] &&
      firstRow[0].toString().startsWith("library_row_");
    ...
    if (isFirstRowData) {
      ...
      dataRows = rows;
      console.log(
        "📊 No header row detected - treating all rows as data:",
        dataRows.length
      );
    } else {
    ...
    const ptrItems: PTRItem[] = dataRows.map((row, index) => {
      ...
      return {
        ...
        daysSinceLastSent,
        isReadyForRotation:
          daysSinceLastSent === null || daysSinceLastSent >= 14,
    ...
    if (endpoint === "daily-status") {
      ...
      const readyForRotation = ptrItems.filter(
        (item) => item.isReadyForRotation
      );
