      ...
      try {
        ...
        for (const tier of selectedTiers) {
          ...
          for (const sheetType of sheetTypes) {
            currentStep++;
            const sheetTypeCapitalized = sheetType.charAt(0).toUpperCase() + sheetType.slice(1);
            ...
            const templateId = TEMPLATE_SHEETS[sheetType as keyof typeof TEMPLATE_SHEETS];
            ...
            const sheetUrl = copiedFile.data.webViewLink || `https://docs.google.com/spreadsheets/d/${sheetId}`;

            if (!sheetId) {
              throw new Error(`Failed to copy ${tier} ${sheetTypeCapitalized} sheet`);
            }

            console.log(`✅ Created ${tier} ${sheetTypeCapitalized} Sheet:`, sheetId);
        ...
        sendEvent('error', { message: error.message || 'Failed to generate POD sheets' });
        controller.close();
      }
    },
  });
