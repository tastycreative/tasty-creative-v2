    ...
    async start(controller) {
      ...
      try {
        ...
        if (!clientModel.launchesPodFolderId) {
          sendEvent("error", {
            message: "Model does not have a launches pod folder configured",
          });
          controller.close();
          return;
        }

        const launchesFolderId = extractFolderId(
          clientModel.launchesPodFolderId
        );
        if (!launchesFolderId) {
        ...
        sendEvent("progress", {
          step: "folder",
          message: "Searching for CAPTION BANK folder",
        });
        ...
        if (
          ...
          sendEvent("progress", {
            step: "folder",
            message: "Creating CAPTION BANK folder",
          });
        ...
        if (!captionBankFolderId) {
          sendEvent("error", {
            message: "Failed to get CAPTION BANK folder ID",
          });
          controller.close();
          return;
        }
        ...
        sendEvent("progress", {
          step: "copy",
          message: "Copying template sheet",
        });
        ...
        if (selectedTypes.length > 0) {
          ...
          console.log(
            ...
            JSON.stringify(duplicateRequests, null, 2)
          );
          ...
          console.log(
            ...
            JSON.stringify(batchUpdateResponse.data, null, 2)
          );

          // Step 5: Update merged cell A-E with model name and tab type
          sendEvent("progress", {
            step: "update",
            message: "Updating tab headers with model name",
          });
          ...
          if (schedulerUrls.free || schedulerUrls.paid) {
            sendEvent("progress", {
              step: "formulas",
              message: "Adding scheduler import formulas",
            });
          ...
          sendEvent("progress", {
            step: "protect",
            message: "Protecting cells in new tabs",
          });
          ...
          sendEvent("progress", {
            step: "formulas",
            message: "Updating MasterSheet DB formulas",
          });
          ...
          const mmSheet = sheetsList.find(
            (s) => s.properties?.title === "MasterSheet DB (MM)"
          );
          const postSheet = sheetsList.find(
            (s) => s.properties?.title === "MasterSheet DB (POST)"
          );
        ...
        sendEvent("error", {
          message: error.message || "Failed to generate sheet",
        });
        controller.close();
      }
    },
