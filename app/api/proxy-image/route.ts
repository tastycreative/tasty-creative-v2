...
export async function GET(request: NextRequest) {
  try {
      ...
      if (policy) {
        try {
          const decodedPolicy = JSON.parse(
            Buffer.from(policy, "base64").toString()
          );
    ...
    if (!response.ok) {
      console.error(
        `Image fetch failed: ${response.status} ${response.statusText} for URL: ${imageUrl}`
      );
      ...
      if (response.status === 403) {
        console.error(
          "OnlyFans CDN returned 403 Forbidden - likely due to IP restrictions or expired authentication"
        );

        // Check if it's a CloudFront authentication error
        if (
          responseText.includes("MissingKey") ||
          responseText.includes("Key-Pair-Id")
        ) {
    ...
    if (!contentType.startsWith("image/")) {
      console.error(
        `Invalid content type: ${contentType}. Expected image, got:`,
        contentType
      );
      ...
      return NextResponse.json(
        { error: "Invalid image response from CDN" },
        { status: 400 }
      );
    }
