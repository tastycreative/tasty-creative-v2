...
async function fetchWithRetry(sheets, spreadsheetId, ranges, retryCount = 0) {
  ...
  try {
    ...
    if (error.code === 429 && retryCount < maxRetries) {
      ...
      console.log(
        `Quota exceeded, retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`
      );
...
export async function GET() {
  try {
    ...
    const createPricingGroup = (
      ...
      const itemNames = headersRow
        .slice(itemStartCol - 1, itemEndCol)
        .filter(Boolean);
          ...
          if (creatorRow) {
            ...
            itemNames.forEach((itemName, itemIndex) => {
              if (itemName && itemName.trim()) {
                ...
                if (price && price.trim()) {
                  creatorPricingMap[trimmedName][itemName.trim()] =
                    price.trim();
                }
              }
            });
