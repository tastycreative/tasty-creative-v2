...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `📊 All Content API: Fetching data for ${modelName}, page ${page}, limit ${limit}`
    );
    ...
    const vaultCount = (vaultCountRes.data.values || []).filter(
      (row) => row[0] && row[0].trim() !== ""
    ).length;
    const sextingCount = (sextingCountRes.data.values || []).filter(
      (row) => row[0] && row[0].trim() !== ""
    ).length;
    const socialCount = (socialCountRes.data.values || []).filter(
      (row) => row[0] && row[0].trim() !== ""
    ).length;
    ...
    console.log(
      `📊 Sheet counts - Vault: ${vaultCount}, Sexting: ${sextingCount}, Social: ${socialCount}, Total: ${totalItems}`
    );
    ...
    vaultRows.forEach((row: string[], index: number) => {
      ...
      const folderIdMatch2 = videoLink?.match(
    ...
    sextingRows.forEach((row: string[], index: number) => {
      ...
      const folderIdMatch1 = sextingSetLink?.match(
        /\/folders\/([a-zA-Z0-9-_]+)/
      );
      const folderIdMatch2 = sextingSetLink?.match(
        /\/drive\/folders\/([a-zA-Z0-9-_]+)/
      );
    ...
    allItems.sort((a, b) => {
      const dateA = new Date(
        a.creationDate || a.uploadDate || "1970-01-01"
      ).getTime();
      const dateB = new Date(
        b.creationDate || b.uploadDate || "1970-01-01"
      ).getTime();
      return dateB - dateA;
    });
    ...
    console.log(
      `📊 All Content API: Processed ${allItems.length} items, returning ${paginatedItems.length} for page ${page}`
    );
    console.log(
      `📁 Folder items in response:`,
