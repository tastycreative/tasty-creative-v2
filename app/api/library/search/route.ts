...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session?.refreshToken) {
      console.warn(
        "No refresh token available - API calls may fail when access token expires"
      );
    ...
    try {
      ...
      for (const range of possibleRanges) {
        try {
          ...
          console.log(
            `Success with range: ${range}, rows: ${response.data.values?.length || 0}`
          );
          break;
        } catch (error) {
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Check if the sheet exists and you have permission to access it",
        },
        { status: 500 }
      );
    }
    ...
    let items = dataRows.map((row, index) => {
      const contentStyle =
        getColumnValue(row, "content style") || getColumnValue(row, "style");
      const creatorName =
        getColumnValue(row, "creator") || getColumnValue(row, "model");
      const captionText = getColumnValue(row, "caption") || "";
      const priceStr = getColumnValue(row, "price") || "0";
      const price = parseFloat(priceStr.replace(/[^0-9.]/g, "")) || 0;

      const performanceStr =
        getColumnValue(row, "performance") || getColumnValue(row, "buys") || "";
      const { totalBuys, totalRevenue } =
        parsePerformanceMetrics(performanceStr);
      ...
      return {
        ...
        contentStyle,
        messageType:
          getColumnValue(row, "message type") || getColumnValue(row, "type"),
        gifUrl:
          getColumnValue(row, "content") || getColumnValue(row, "preview"),
        previewUrl:
          getColumnValue(row, "content") || getColumnValue(row, "preview"),
        creatorName,
      };
...
function parsePerformanceMetrics(performanceStr: string): {
  totalBuys: number;
  totalRevenue: number;
} {
  ...
  const totalRevenue = revenueMatch
    ? parseFloat(revenueMatch[1].replace(/,/g, ""))
    : 0;
...
function determineCategoryFromStyle(contentStyle: string): string {
  ...
  if (
    style.includes("masturbation") ||
    style.includes("solo") ||
    style.includes("pussy play")
  )
    return "Solo";
  if (
    style.includes("blowjob") ||
    style.includes("bj") ||
    style.includes("oral")
  )
    return "BJ";
