  ...
  try {
    ...
    try {
      folderInfo = await drive.files.get({
        fileId: folderId,
        fields:
          "id,name,mimeType,parents,shared,ownedByMe,capabilities,permissions",
      });
    ...
    try {
      ...
      response = await drive.files.list({
        ...
        fields:
          "files(id,name,mimeType,webViewLink,thumbnailLink,videoMediaMetadata,createdTime,size,parents,shared,ownedByMe)",
    ...
    if (folderFiles.length === 0) {
      try {
        console.log(
          "Trying approach 2: Without ordering, with shared drives..."
        );
        response = await drive.files.list({
          q: `'${folderId}' in parents and trashed=false`,
          fields:
            "files(id,name,mimeType,webViewLink,thumbnailLink,videoMediaMetadata,createdTime,size,parents)",
          pageSize: 100,
          includeItemsFromAllDrives: true,
          supportsAllDrives: true,
        });
    ...
    if (folderFiles.length === 0) {
      try {
        ...
        response = await drive.files.list({
          ...
          fields:
            "files(id,name,mimeType,webViewLink,thumbnailLink,videoMediaMetadata,createdTime,size)",
          pageSize: 100,
          includeItemsFromAllDrives: true,
          supportsAllDrives: true,
        });
    ...
    if (folderFiles.length === 0) {
      console.log(
        `No files found in folder ${folderId}. This could be an empty folder or a permissions issue.`
      );
    }
