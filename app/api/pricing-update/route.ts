...
export async function POST(request: NextRequest) {
  try {
    ...
    const { creatorName, itemName, newPrice, creatorRowNumber } =
      await request.json();
    ...
    console.log(
      ...
      JSON.stringify(headersResponse.data, null, 2)
    );
    ...
    if (headers.length === 0) {
      console.log(
        "No headers found in Client Info!A1:BB1, trying just Client Info!A1:Z1..."
      );
    ...
    const itemColumnIndex = headers.findIndex(
      (header) =>
    ...
    if (!creatorRowIndex) {
      ...
      creatorRowIndex =
        creatorNames.findIndex(
          (name) =>
            name &&
            name.trim().toLowerCase() === creatorName.trim().toLowerCase()
        ) + 1;
    ...
    console.log(
      `Updating cell: Client Info!${columnLetter}${creatorRowIndex} with value: "${newPrice}"`
    );
