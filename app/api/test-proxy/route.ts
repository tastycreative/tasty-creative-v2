...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!testUrl) {
      ...
      return NextResponse.json({
        ...
        usage: {
          ...
          cleanup: "/api/proxy-image/cleanup (GET for status, DELETE for cleanup)",
        },
        features: [
    ...
    if (type === "video") {
      ...
      const videoResponse = await fetch(`${request.nextUrl.origin}/api/proxy-video?url=${encodeURIComponent(testUrl)}`, {
        method: "HEAD",
      });
      ...
      // Test image proxy
      const imageResponse = await fetch(`${request.nextUrl.origin}/api/proxy-image?url=${encodeURIComponent(testUrl)}`);
      ...
      return NextResponse.json({
        ...
        cacheStatus,
        imageSize: imageSize ? `${Math.round(parseInt(imageSize) / 1024)} KB` : "unknown",
