...
export const uploadToS3 = async (
  ...
  return {
    key,
    url: `https://${process.env.S3_BUCKET1}.s3.${process.env.S3_REGION}.amazonaws.com/${key}`,
  };
};
...
export const listUserFiles = async (userId: string, folder?: string) => {
  ...
  return (
    response.Contents?.filter(
      (item) => !item.Key!.endsWith(".placeholder")
    ).map((item) => ({
      key: item.Key!,
      size: item.Size!,
      lastModified: item.LastModified!,
      url: `https://${process.env.S3_BUCKET1}.s3.${process.env.S3_REGION}.amazonaws.com/${item.Key}`,
    })) || []
  );
};
