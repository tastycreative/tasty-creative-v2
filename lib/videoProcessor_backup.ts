...
const generateFrameOptimized = async (
  const ctx = canvas.getContext("2d");
  ...
  // Preload all videos with enhanced loading and validation
  await Promise.all(
    videos.map(
      (video) =>
        new Promise<void>((resolve, reject) => {
          ...
          videoElement.preload = "auto"; // Load full video data

          // Ensure video is ready for seeking by waiting for canplaythrough
          videoElement.oncanplaythrough = () => {
            // Additional validation - seek to middle and back to start
            const validateVideo = async () => {
              try {
                // Test seek to middle of video
                const midTime = videoElement.duration / 2;
                videoElement.currentTime = midTime;

                await new Promise<void>((seekResolve) => {
                  const waitForSeek = () => {
                    if (
                      Math.abs(videoElement.currentTime - midTime) < 0.1 ||
                ...
                if (
                  ...
                  console.log(
                    `Video ${video.file.name} loaded and validated successfully`
                  );
                  resolve();
                } else {
                  reject(
                    new Error(`Video ${video.file.name} has invalid dimensions`)
                  );
                }
              } catch (error) {
                reject(
                  new Error(
                    `Video validation failed for ${video.file.name}: ${error}`
                  )
                );
              }
            };
          ...
          setTimeout(() => {
            if (!videoElements[video.id]) {
              reject(new Error(`Video loading timeout: ${video.file.name}`));
            }
          }, 15000); // Increased timeout for full loading
        })
    )
  );
  ...
  while (frameIndex < totalFrames) {
    ...
    // Wait for all frames in batch
    const batchFrames = await Promise.all(batchPromises);

    // Write batch to FFmpeg and update last valid frame
    for (const frame of batchFrames) {
      const filename = `frame${frame.index.toString().padStart(6, "0")}.jpg`;
      await ffmpeg.FS("writeFile", filename, frame.data);

      // Store a copy of canvas for frame interpolation if needed
      if (frame.index % 5 === 0) {
        // Store every 5th frame
        if (!lastValidFrame) {
          lastValidFrame = document.createElement("canvas");
          lastValidFrame.width = settings.width;
          lastValidFrame.height = settings.height;
        }
        const lastCtx = lastValidFrame.getContext("2d");
        if (lastCtx) {
    ...
    // Report progress (15-70% for frame generation)
    if (onProgress) {
      onProgress(15 + (frameIndex / totalFrames) * 55);
    }
  ...
  console.log(`Generated ${totalFrames} frames, now encoding with FFmpeg...`);

  // Prepare optimized FFmpeg command
  const outputFilename = `output.${settings.format}`;
  const inputPattern = "frame%06d.jpg"; // JPEG input

  // Calculate optimized bitrate
  const pixelCount = settings.width * settings.height;
  const baseBitrate = Math.min(
    6000,
    Math.max(1500, Math.floor((pixelCount * 1.5) / 1000))
  );
  const qualityMultiplier = settings.quality / 100;
  const targetBitrate = Math.floor(baseBitrate * qualityMultiplier);
  ...
  if (settings.format === "mp4") {
    ffmpegArgs = [
      ...
      `${targetBitrate}k`,
      "-preset",
      "fast", // Changed from "medium" to "fast"
      "-tune",
      "stillimage", // Optimize for image sequence
      "-movflags",
      "+faststart",
    ...
    ffmpegArgs = [
      ...
      `${targetBitrate}k`,
      "-crf",
      "35", // Slightly lower quality for speed
      "-speed",
      "4", // Faster encoding
      "-tile-columns",
      "2",
  ...
  // Quick cleanup (don't wait for all files)
  setTimeout(() => {
    for (let i = 0; i < totalFrames; i++) {
      const filename = `frame${i.toString().padStart(6, "0")}.jpg`;
      try {
...
const generateFrameOptimized = async (
  ...
  settings: ExportSettings,
  lastValidFrame?: HTMLCanvasElement | null
): Promise<{ index: number; data: Uint8Array }> => {
  // Find current video for this time
    ...
    if (
      ...
      const videoTime = Math.max(
        ...
        Math.min(
          ...
          videoElement.duration - 0.1 // Give more buffer to avoid edge cases
        )
      );

      // Improved seeking with better validation
      const timeDiff = Math.abs(videoElement.currentTime - videoTime);
      if (timeDiff > 0.1) {
        // Only seek if time difference is significant
        videoElement.currentTime = videoTime;

        // More robust waiting for seek completion
        await new Promise<void>((resolve) => {
          ...
          const waitForSeek = () => {
            ...
            if (
              (currentTimeDiff < 0.2 && isVideoReady && hasValidFrame) ||
      ...
      const hasValidDimensions =
        videoElement.videoWidth > 0 && videoElement.videoHeight > 0;
      const isNotSeeking = !videoElement.seeking;

      if (isVideoReady && hasValidDimensions && isNotSeeking) {
        try {
          // Test if we can actually draw the frame by checking for valid pixel data
          ...
          if (testCtx) {
            testCtx.drawImage(videoElement, 0, 0, 1, 1);
            const testData = testCtx.getImageData(0, 0, 1, 1).data;
            const hasValidPixels =
              testData[0] > 0 || testData[1] > 0 || testData[2] > 0;

            if (hasValidPixels) {
              // Frame is valid, apply effects
              applyVideoEffects(
                videoElement,
                canvas,
              ...
              console.warn(
                `Frame ${frameIndex} has no valid pixels, using fallback`
              );
              // Fallback: draw a simple frame without effects
              drawVideoFallback(
                videoElement,
                canvas,
                ctx,
            ...
            // Fallback if test context fails
            drawVideoFallback(
              videoElement,
              canvas,
              ctx,
          ...
          console.warn(`Error processing frame ${frameIndex}:`, error);
          // Fallback drawing
          drawVideoFallback(
            videoElement,
            canvas,
        ...
        console.warn(
          `Frame ${frameIndex} skipped - video not ready (readyState: ${videoElement.readyState}, seeking: ${videoElement.seeking})`
        );
        // Keep black frame but log the issue
      }
    }
  }

  // Validate that we have some content before returning
  const imageData = ctx.getImageData(0, 0, settings.width, settings.height);
  const hasContent = imageData.data.some((value, index) => {
    // Check RGB values (skip alpha channel)
    return index % 4 !== 3 && value > 10; // Use threshold > 10 to ignore very dark pixels
  });

  if (!hasContent && currentVideo) {
    console.warn(
      `Frame ${frameIndex} appears to be completely black despite having video content`
    );

    // Try to use last valid frame as fallback to reduce black frames
    if (lastValidFrame) {
      const lastValidCtx = lastValidFrame.getContext("2d");
      if (lastValidCtx) {
        console.log(
          `Using last valid frame as fallback for frame ${frameIndex}`
        );
        ctx.drawImage(lastValidFrame, 0, 0);

        // Add a slight overlay to indicate this is an interpolated frame
  ...
  return new Promise<{ index: number; data: Uint8Array }>((resolve) => {
    canvas.toBlob(
      (blob) => {
        if (blob) {
          blob.arrayBuffer().then((buffer) => {
            resolve({ index: frameIndex, data: new Uint8Array(buffer) });
          });
        } else {
...
// Helper function for fallback drawing
const drawVideoFallback = (
  ...
  try {
    ...
    console.warn("Fallback draw failed:", error);
    // Ultimate fallback: draw a gray rectangle to indicate missing frame
